...
// Helper to generate and shuffle deck
...
io.on('connection', (socket) => {
    ...
    socket.on('playCard', ({ roomID, cardObject }) => {
        ...
        if (room.table.length > 0 && playedCard.suit !== room.currentSuit) {
            if (player.hand.some(c => c.suit === room.currentSuit)) {
                return socket.emit('errorMsg', `Follow suit: ${room.currentSuit}`);
            }
        }
        ...
        if (isPangkah || room.table.length === activeInRound) {
            setTimeout(() => {
                ...
                if (survivors.length <= 1) {
                    ...
                    io.to(roomID).emit('clearTable', { turn: room.turn, winner: room.players[winnerIdx].name, players: room.players, msg: isPangkah ? "Pangkah!" : "Round Over" });
                }
            }, 1500);
        } else {
    ...
    socket.on('disconnect', () => {
        // Find room and cleanup if necessary
    });
});
